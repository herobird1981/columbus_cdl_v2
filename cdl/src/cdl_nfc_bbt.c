...
/*****************************************************************************/
//  Function:  BOOL nfc_bbt_va_to_pa(bbt_t *p_bbt, uint32_t va, uint32_t *p_pa)
//
//  Parameters:
...
{
	nfc_para_t_ptr p_nfc_para = &g_nfc_para;
	uint16_t tmp_vb, vb, blk, page;
	uint16_t byte_idx, byte_ofst;

	vb = va >> p_nfc_para->blk_shift_bits;
	page = va & ((1 << p_nfc_para->blk_shift_bits) - 1);
	...
	for (blk = 0; blk < p_nfc_para->blks_per_chip; blk++)
	{
		...
		if ((p_bbt->data[byte_idx] & (3 << byte_ofst)) == (BBT_TYPE_GOOD << byte_ofst))
		{
			if (tmp_vb++ == vb)
			{
				*p_pa = (blk << p_nfc_para->blk_shift_bits) + page;
				return TRUE;
			}
		}
	}
...
/*****************************************************************************/
//  Function:  void nfc_bbt_pa_to_va(bbt_t *p_bbt, uint32_t pa, uint32_t *p_va)
//
//  Parameters:
//		p_bbt [in]	: data buffer which is to store the bad block table(bbt)
//						  2 bits indicate a block type 
...
BOOL nfc_bbt_pa_to_va(bbt_t *p_bbt, uint32_t pa, uint32_t *p_va)
{
	...
	pb = pa >> p_nfc_para->blk_shift_bits;
	page = pa & ((1 << p_nfc_para->blk_shift_bits) - 1);

	// Check the physical block range
	if (pb >= p_nfc_para->blks_per_lun)
	{
		*p_va = 0xFFFFFFFF;
		return FALSE;
	}
	...
	*p_va = ((vb - 1) << p_nfc_para->blk_shift_bits) + page;
	return TRUE;
}

/*****************************************************************************/
//  Function:   uint8_t static nfc_bbt_blk_chk(uint32_t blk)
//
//  Parameters:
//		blk					: flash block address
//
//  Return:
// 		NFC_ST_DONE		: valid block
//
//		NFC_ST_FAIL			: invalid block
//
// 		NFC_ST_TIME_OUT		: bad block checks timeout
//
//  Description:
...
uint8_t nfc_bbt_blk_chk(uint32_t blk)
{
	...
	debug("check blk 0x%x\n", blk);
	
	// check bad block mode
	page[0] = 0;
	page[1] = 1;
	page[2] = p_nfc_para->pages_per_blk - 1;

	for (i = 0; i < 3; i++)
	{
		ret_val = nfc_spare_read(((blk<<p_nfc_para->blk_shift_bits)+page[i]), spare_data, 4);
		...
		else if (ret_val == NFC_ST_TIME_OUT) 
		{
...
/*****************************************************************************/
//  Function:   uint8_t nfc_bbt_scan(bbt_t *p_bbt)
//
//  Parameters:
//		p_bbt [in]		: data buffer which is to store the bad block table(bbt)
//						  2 bits indicate a block type 
...
//
//  Return:
//		NFC_ST_DONE	: scan bbt ok
//
//		NFC_ST_FAIL		: scan bbt fail
//
//		NFC_ST_TIME_OUT	: scan bbt timeout
//
//  Description:
...
uint8_t nfc_bbt_scan(bbt_t *p_bbt)
{
	...
	{	
		...
		ret_val = nfc_bbt_blk_chk(blk);
		if (ret_val == NFC_ST_FAIL)
		{
			// marked as bad block
			p_bbt->data[byte_idx] |= (BBT_TYPE_BAD << byte_ofst); 
		}
		else if (ret_val == NFC_ST_DONE)
		{
...
/*****************************************************************************/
//  Function:   uint8_t nfc_bbt_write(bbt_t *p_bbt)
//
//  Parameters:
//		p_bbt [in]		: data buffer which is to store the bad block table(bbt)
//						  2 bits indicate a block type 
...
//
//  Return:
//		NFC_ST_DONE	: write bbt ok
//
//		NFC_ST_FAIL		: write bbt fail
//
//		NFC_ST_TIME_OUT	: write bbt timeout
//
//  Description:
...
uint8_t nfc_bbt_write(bbt_t *p_bbt)
{
	...
	i = 0;
	for (blk = p_nfc_para->blks_per_chip - 1; blk >= 0; blk--)
	{
		byte_idx = blk >> 2;
		byte_ofst = (blk & 0x3) << 1;		
	...
	for (i = 0; i < 2; i++)
	{
		addr = bbt_blk[i]<<p_nfc_para->blk_shift_bits;
		
		// erase the block
		ret_val = nfc_blk_erase(addr);
		if (ret_val != NFC_ST_DONE)
		{
			return ret_val;
		}
		...
		ret_val = nfc_page_write(addr, (uint8_t *)p_bbt, p_nfc_para->page_size);
		if (ret_val != NFC_ST_DONE)
		{
			return ret_val;
		}		
...
/*****************************************************************************/
//  Function:   uint8_t nfc_bbt_read(bbt_t *p_bbt)
//
//  Parameters:
...
//
//  Return:
//		NFC_ST_DONE	: read bbt ok
//
//		NFC_ST_FAIL		: read bbt fail
//
//		NFC_ST_TIME_OUT	: read bbt timeout
//
//  Description:
...
uint8_t nfc_bbt_read(bbt_t *p_bbt)
{
	...
	for (blk = p_nfc_para->blks_per_chip - 1; blk >= 0; blk--)
	{		
		ret_val = nfc_page_read((blk << p_nfc_para->blk_shift_bits), (uint8_t *)p_bbt, p_nfc_para->page_size);
		if (ret_val == NFC_ST_DONE)
		{
...
/*****************************************************************************/
//  Function:   void nfc_bbt_blk_marked(bbt_t *p_bbt, uint32_t blk, E_BBT_TYPE type)
//
//  Parameters:
...
/*****************************************************************************/
//  Function:   E_BBT_TYPE nfc_bbt_blk_verify(bbt_t *p_bbt, uint32_t blk)
//
//  Parameters:
//		p_bbt [in]		: data buffer which is to store the bad block table(bbt)
//						  2 bits indicate a block type 
...
uint8_t nfc_bbt_test(void)
{
	...
	for (blk = 0; blk < p_nfc_para->blks_per_chip; blk++)
	{
		ret_val = nfc_blk_erase(blk<<p_nfc_para->blk_shift_bits);
	...
	//nfc_bbt_blk_marked(p_bbt, 0, BBT_TYPE_BAD);
	//nfc_bbt_blk_marked(p_bbt, 1, BBT_TYPE_BAD);
	//nfc_bbt_blk_marked(p_bbt, 2, BBT_TYPE_BAD);
	nfc_bbt_blk_marked(p_bbt, 3, BBT_TYPE_BAD);
	nfc_bbt_blk_marked(p_bbt, 4, BBT_TYPE_BAD);
	//nfc_bbt_blk_marked(p_bbt, (p_nfc_para->blks_per_chip - 1), BBT_TYPE_BAD);
	//nfc_bbt_blk_marked(p_bbt, (p_nfc_para->blks_per_chip - 2), BBT_TYPE_BAD);
#endif
	...
	for (blk = 0; blk < p_nfc_para->blks_per_chip; blk++)
	{
		pa = blk << p_nfc_para->blk_shift_bits;
		nfc_bbt_pa_to_va(p_bbt, pa, &va);
