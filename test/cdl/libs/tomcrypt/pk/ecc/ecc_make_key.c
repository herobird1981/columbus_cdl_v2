...
int ecc_make_key(prng_state *prng, int wprng, int keysize, ecc_key *key)
{
   ...
   err = ecc_make_key_ex(prng, wprng, key, &ltc_ecc_sets[x], 0, NULL);
   key->idx = x;
   return err;
}

int ecc_make_key_ex(prng_state *prng, int wprng, ecc_key *key, const ltc_ecc_set_type *dp, int bak_key, unsigned char *buf_sp)
{
   int            err;
   ecc_point     *base;
   void          *prime, *order;
   ...
   if(buf_sp) {
	   if(bak_key)
		   memcpy(buf_sp, buf, keysize);
	   else
		   memcpy(buf, buf_sp, keysize);
   }
