 ...
 */
/*#define MGC_MSD_BIDIR_ENDPOINTS*/  /*sunny remove*/
...
#define MGC_aMsdData (0x42000000)
#define MGC_dwMsdBlockCount (128*1024*1024/512) /*128M*/
...
static void MGC_MsdCommand(const MGC_MsdCbw* pCbw)
{
	...
	MGC_MsdTxDataIrp.pBuffer = (uint8_t *)(MGC_aMsdData + dwBlock * MGC_wBlockSize);  /*&(MGC_aMsdData[dwBlock * MGC_wBlockSize]);*/
	...
	dwLength = pCbw->aCbwCb[4];
	dwBlock = (pCbw->aCbwCb[2] << 8) | pCbw->aCbwCb[3];
	MGC_MsdRxDataIrp.pBuffer = (uint8_t*)(MGC_aMsdData + dwBlock * MGC_wBlockSize);  /*&(MGC_aMsdData[dwBlock * MGC_wBlockSize]);*/
