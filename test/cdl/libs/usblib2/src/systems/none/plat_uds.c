...
static void* MGC_NoneInitController(const MUSB_NoneController* pControllerInfo)
{
	...
	do
	{
	    ...
	    /* try UCD init */
	    pSystem->pController = MUSB_NewController(&MGC_gNoneUtils, pControllerInfo->wType,
	    	    pControllerInfo->dwHighSpeed, pControllerInfo->dwSendSrp, pBaseIsr, pBaseBsr);
 ...
 */
uint8_t MUSB_InitSystem(unsigned long dwBsrPriority, void *regs, int irq, int dwHighSpeed, int dwSendSrp)
{
    MGC_NoneSystem* pSystem;
    uint8_t bIndex;
	...
	bIndex++)
    {
        MUSB_aNoneController[0].pBase = regs;
        MUSB_aNoneController[0].dwInfo = irq;
        MUSB_aNoneController[0].dwHighSpeed = dwHighSpeed;
        MUSB_aNoneController[0].dwSendSrp = dwSendSrp;
