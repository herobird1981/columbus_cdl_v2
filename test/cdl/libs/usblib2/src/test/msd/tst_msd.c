...
static void MGC_TestMsdStart(MGC_TestFsDevice* pTestFsDevice)
{
	...
	MUSB_PrintLine("  P: Host Suspend Test");
	...
	else if(('P' == aAnswer[0]) || ('p' == aAnswer[0]))
	{
		MUSB_PrintLine("Host suspend the bus");
		MUSB_SuspendBus(MGC_hCdiBus);
		return;
	}
...
static void MGC_CheckMedium(void)
{
...
/* Entrypoint */
int msd_main(void *regs, int irq, int dwHighSpeed, int dwSendSrp)
{
    ...
    if(!MUSB_InitSystem(5, regs, irq, dwHighSpeed, dwSendSrp))
    {
    ...
    while(TRUE)
    {
        if (console_tstc()) {
            cc = console_getc();
            if (cc == 'q' || cc == 0x03) {
                MUSB_PrintLine("\rCTRL+C or Quit - Operation aborted.\n");
                break;
            }
        }
