...
uint8_t MGC_HdrcInit(MGC_Port* pPort)
{
    ...
    info("ConfigData=0x%02x\n", config);
#ifndef MUSB_DISABLE_BULK_COMBINE_SPLIT
...
#else
	pPort->bHasBulkCombine = FALSE;
	pPort->bHasBulkSplit = FALSE;
#endif
...
uint32_t MGC_HdrcStop(MGC_Controller* pController)
{
    ...
    /*info("MGC_HdrcStop: disable and flush interrupt\n");*/
    /* disable interrupts */
...
uint32_t MGC_HdrcReadBusState(MGC_Port* pPort)
{
    ...
    uint8_t* pBase = (uint8_t*)pController->pControllerAddressIst;
    /*info("ReadBusState start: power=0x%02x, devctrl=0x%02x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER), MGC_Read8(pBase, MGC_O_HDRC_DEVCTL));*/
    ...
    info("ReadBusState end: power=0x%02x, devctrl=0x%02x dwFrame=%d\n", 
		MGC_Read8(pBase, MGC_O_HDRC_POWER), MGC_Read8(pBase, MGC_O_HDRC_DEVCTL), pPort->dwFrame);
...
uint32_t MGC_HdrcProgramBusState(MGC_Port* pPort)
{
    ...
    info("ProgramBusState start: power=0x%02x, devctrl=0x%02x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER), MGC_Read8(pBase, MGC_O_HDRC_DEVCTL));
    ...
    else if (MGC_Read8(pBase, MGC_O_HDRC_DEVCTL)&0x80)
    {
	nDevCtl &= ~MGC_M_DEVCTL_SESSION;
    }
    ...
    {
	MGC_DIAG1(2, pController, "!!!Updating FADDR to ", pPort->bFuncAddr, 10, 0);
	MGC_Write8(pBase, MGC_O_HDRC_FADDR, pPort->bFuncAddr);
    }
    if(nPower != power)
    {
	MGC_DIAG1(2, pController, "!!!Updating POWER to ", nPower, 16, 2);
	MGC_Write8(pBase, MGC_O_HDRC_POWER, nPower);
    }
    if(nDevCtl != devctl)
    {
	MGC_DIAG1(2, pController, "!!!Updating DevCtl to ", nDevCtl, 16, 2);
	MGC_Write8(pBase, MGC_O_HDRC_DEVCTL, nDevCtl);
    }
    info("ProgramBusState end: power=0x%02x, devctrl=0x%02x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER), MGC_Read8(pBase, MGC_O_HDRC_DEVCTL));
    return 0;
}
			       ...
			       uint8_t bDir, uint8_t bReuse)
{
    ...
    if(bTx)
    {
	MGC_DIAG1(3, pController, "Flushing Tx on ", bEnd, 16, 2);
#ifdef MUSB_DMA
    ...
    {
	MGC_DIAG1(3, pController, "Flushing Rx on ", bEnd, 16, 2);
	if(pPort->bIsHost)
	{
	    if(bEnd)
	    {
...
uint8_t MGC_HdrcServiceDefaultEnd(MGC_Port* pPort, MGC_BsrItem* pItem)
{
    ...
    if(pPort->bIsHost)
    {
	if (pPort->bDisconnect)
	{
	    MGC_DIAG(3, pPort->pController, "disconnected\n");
	...
	if(pPort->bWantTestMode)
	{
		...
		MGC_DIAG(2, pPort->pController, "MUSB_TESTMODE_TEST_PACKET\n");
...
static void MGC_HdrcResumeOff(void* pParam, uint16_t wTimer)
{
    ...
    info("ResumeOff end: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
...
void MGC_HdrcSetPortTestMode(MGC_Port* pPort, MUSB_HsetPortMode eMode)
{
	...
	/* despite explicit instruction, we still must kick-start */
	MGC_WriteCsr16(pBase, MGC_O_HDRC_CSR0, 0, MGC_M_CSR0_TXPKTRDY);
	info("power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
			MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
			MGC_Read8(pBase, (0x10 + MGC_O_HDRC_CSR0)));
	break;
	...
	info("\nSUSPEND start: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	bReg = MGC_Read8(pBase, MGC_O_HDRC_POWER);
	MGC_Write8(pBase, MGC_O_HDRC_POWER, bReg | MGC_M_POWER_SUSPENDM);
	info("SUSPEND end: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	break;
    case MUSB_HSET_PORT_RESUME:
	info("\nRESUME start: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	bReg = MGC_Read8(pBase, MGC_O_HDRC_POWER) & ~MGC_M_POWER_SUSPENDM;
	MGC_Write8(pBase, MGC_O_HDRC_POWER, bReg | MGC_M_POWER_RESUME);
	info("RESUME end: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	pController->pSystemServices->pfArmTimer(
	...
	info("SETUP start: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	/* clear endpoint 0 interrupts as a cheat for single-step mode */
	MGC_Write16(pBase, MGC_O_HDRC_INTRTX, wReg & ~0xfffe);
	MGC_SelectEnd(pBase, 0);
	MGC_WriteCsr16(pBase, MGC_O_HDRC_CSR0, 0,
		MGC_M_CSR0_TXPKTRDY | MGC_M_CSR0_H_SETUPPKT | MGC_M_CSR0_H_NO_PING);
	info("SETUP end: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	break;
    case MUSB_HSET_PORT_SETUP_IN:
	info("\nDATAIN start: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	/* clear endpoint 0 interrupts as a cheat for single-step mode */
	MGC_Write16(pBase, MGC_O_HDRC_INTRTX, wReg & ~0xfffe);
	MGC_SelectEnd(pBase, 0);
	MGC_WriteCsr16(pBase, MGC_O_HDRC_CSR0, 0, MGC_M_CSR0_H_REQPKT | MGC_M_CSR0_H_NO_PING);
	/*wait for RxPktRdy*/
	for (i=0; i<200; i++) {
        if (MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0) & MGC_M_CSR0_RXPKTRDY)
            break;
        if (i == 200)
            info("timeout for RxPktRdy\n");
	}
	/*read fifo*/
	wCount = MGC_ReadCsr8(pBase, MGC_O_HDRC_COUNT0, 0);
	MGC_HdrcUnloadFifo(pPort, 0, wCount, aSetupRx);

	/*clear CSR0_RXPKTRDY*/
	wCsr = MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0);
	wCsr &= ~MGC_M_CSR0_RXPKTRDY;
	MGC_WriteCsr16(pBase, MGC_O_HDRC_CSR0, 0, wCsr);
	...
	info("\n");
	info("DATAIN end: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	break;
    case MUSB_HSET_PORT_SETUP_STATUS:
	info("STATUS start: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	/* clear endpoint 0 interrupts as a cheat for single-step mode */
	MGC_Write16(pBase, MGC_O_HDRC_INTRTX, wReg & ~0xfffe);
	MGC_SelectEnd(pBase, 0);
	MGC_WriteCsr16(pBase, MGC_O_HDRC_CSR0, 0,
		MGC_M_CSR0_TXPKTRDY | MGC_M_CSR0_H_STATUSPKT | MGC_M_CSR0_H_NO_PING);
	info("STATUS: power:0x%x devctl 0x%x csr0 0x%x\n", MGC_Read8(pBase, MGC_O_HDRC_POWER),
		MGC_Read8(pBase, MGC_O_HDRC_DEVCTL),
		MGC_ReadCsr16(pBase, MGC_O_HDRC_CSR0, 0));
	break;
    }
