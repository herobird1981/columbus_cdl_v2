...
uint8_t MGC_FunctionParseSetup(MGC_Port* pPort, uint8_t* pbStatus)
{
    ...
    *pbStatus = MUSB_STATUS_OK;
    ...
    info("SETUP req%02x.%02x v%04x i%04x l%d\n",
        pRequest->bmRequestType,
        pRequest->bRequest,
        MUSB_SWAP16P((uint8_t*)&pRequest->wValue),
        MUSB_SWAP16P((uint8_t*)&pRequest->wIndex),
        MUSB_SWAP16P((uint8_t*)&pRequest->wLength));
	...
	case MUSB_REQ_SET_FEATURE:
		...
		case MUSB_RECIP_DEVICE:
			...
			*pbStatus = MUSB_STATUS_OK;
		    ...
		    {
			info("\n###B-PERIPHERAL: get SET_FEATURE request: MUSB_b_hnp_enable\n");
	...
	case MUSB_REQ_SET_CONFIGURATION:
		info("MUSB_REQ_SET_CONFIGURATION start bUsbState=%d\n", pPort->bUsbState);
		...
		*pbStatus = MUSB_STATUS_STALLED;
		...
		info("MUSB_REQ_SET_CONFIGURATION end bOK=%d\n", bOK);
	    break;
...
void MGC_FunctionChangeState(MGC_Port* pPort, MUSB_State State)
{
    ...
    {
	MGC_DIAG1(3, pPort->pController, "USB state change: ", (uint32_t)State, 10, 0);
