...
static void MGC_HostSetDeviceAddress(void* pParam, uint16_t wTimer)
{
    ...
    MGC_DIAG1(3, pController, "Setting device address ", 
	pEnumData->bAddress, 10, 0);
    MGC_DIAG1(2, pController, "MGC_HostSetDeviceAddress ", 
    pEnumData->bAddress, 10, 0);
...
static void MGC_HostGetShortDescriptor(void* pParam, uint16_t wTimer)
{
    ...
    MGC_DIAG(3, pController, "Getting 8 bytes of device descriptor");
    MGC_DIAG(2, pController, "MGC_HostGetShortDescriptor:8 bytes");
...
static void MGC_HostGetFullDescriptor(void* pParam, uint16_t wTimer)
{
    ...
    MGC_DIAG(3, pPort->pController, "Getting complete device descriptor");
    MGC_DIAG(2, pPort->pController, "MGC_HostGetFullDescriptor");
...
static void MGC_HostGetConfig(void* pParam, uint16_t wTimer)
{
    ...
    MGC_DIAG1(3, pPort->pController, "Getting config descriptor, length=", 
    	pEnumData->Irp.dwInLength, 16, 4);
    MGC_DIAG1(2, pPort->pController, "MGC_HostGetConfigDescriptor: length=", 
    	pEnumData->Irp.dwInLength, 16, 4);
    ...
    /*info("wLength: %d, dwInLength: %d\n", pRequest->wLength, pEnumData->Irp.dwInLength);*/
    pEnumData->Irp.pOutBuffer = pEnumData->aSetupTx;
...
static void MGC_HostGetDeviceStatus(void* pParam, uint16_t wTimer)
{
    ...
    MGC_DIAG(3, pController, "Getting device status");
    MGC_DIAG(2, pController, "MGC_HostGetDeviceStatus");
...
static void MGC_EhsetGetDescriptorSetup(void* pParam, uint16_t wTimer)
{
    ...
    MGC_DIAG(3, pPort->pController, "MGC_EhsetGetDescriptorSetup");
...
static void MGC_EhsetGetDescriptorIn(void* pParam, uint16_t wTimer)
{
    ...
    MGC_DIAG(3, pPort->pController, "MGC_EhsetGetDescriptorIn");

    pPort->pfSetPortTestMode(pPort, MUSB_HSET_PORT_SETUP_IN);
}

static void MGC_EhsetGetDescriptorStatus(void* pParam, uint16_t wTimer)
{
    MGC_Controller* pController = (MGC_Controller*)pParam;
    MGC_Port* pPort = pController->pPort;

    MGC_DIAG(3, pPort->pController, "MGC_EhsetGetDescriptorStatus");

    pPort->pfSetPortTestMode(pPort, MUSB_HSET_PORT_SETUP_STATUS);
}
...
static void MGC_Ehset(MGC_Port* pPort, uint8_t bBusAddress, uint16_t wPid)
{
	...
	info("wait for 15secs -> suspend -> 15secs delay -> resume \n");
	...
	info("wait 15 secs, perform setup phase of GET_DESC\n");
	pController->pSystemServices->pfArmTimer(
	    pController->pSystemServices->pPrivateData, 0, 15000, FALSE,
	...
	info("perform setup phase of GET_DESC, wait 15 secs, perform IN data phase\n");
	MGC_EhsetGetDescriptorSetup(pController, 0);
	pController->pSystemServices->pfArmTimer(
	    pController->pSystemServices->pPrivateData, 0, 15000, FALSE,
	    MGC_EhsetGetDescriptorIn);

	info("\noption: wait 15 secs, perform status phase\n");
	/* option: wait 15 secs, perform status phase */
	pController->pSystemServices->pfArmTimer(
	    pController->pSystemServices->pPrivateData, 0, 15000, FALSE,
	    MGC_EhsetGetDescriptorStatus);
	break;
    }
}
#endif
...
static void MGC_HostEnumerator(void* pParam, MUSB_ControlIrp* pIrp)
{
	...
	MGC_DIAG1(3, pController, "Got device descriptor; config count=", 
	    pDeviceDesc->bNumConfigurations, 16, 2);
	MGC_DIAG1(3, pController, "Got device descriptor; idVendor ", 
	    pDeviceDesc->idVendor, 16, 2);
	MGC_DIAG1(3, pController, "Got device descriptor; idProduct ", 
	    pDeviceDesc->idProduct, 16, 2);

	pEnumData->bCount = pDeviceDesc->bNumConfigurations;
	pEnumData->bIndex = 0;
	pEnumData->dwData = 0L;

#ifdef MUSB_EHSET  /* sunny add for hset test */
	wIndex = MUSB_SWAP16P((uint8_t*)&(pDeviceDesc->idVendor));
	wValue = MUSB_SWAP16P((uint8_t*)&(pDeviceDesc->idProduct));
	if((0x1a0a == wIndex) && (wValue == 0x0104))
	{
		MGC_Ehset(pPort, pEnumData->pDevice->bBusAddress, wValue);
		return;
	}
	...
	/*MGC_DIAG(2, pController, "Config ", pEnumData->bIndex, " length=", 
	    wLength, 16, 0);*/
	...
	else
	{
	    ...
	    MGC_DIAG1(3, pController, "Total config length=", 
	...
	if(++pEnumData->bIndex >= pEnumData->bCount)
	{
		...
		*/
		...
		info("work with %s\n", pPort->bIsHighSpeed ? "high speed" : "full speed or low speed");
#ifdef MUSB_EHSET  /* sunny add for hset test */
		if((0x1a0a == pEnumData->pDevice->DeviceDescriptor.idVendor) &&
				((pEnumData->pDevice->DeviceDescriptor.idProduct >= 0x0101) &&
				(pEnumData->pDevice->DeviceDescriptor.idProduct <= 0x0108)))
		{
			MGC_Ehset(pPort, pEnumData->pDevice->bBusAddress, pEnumData->pDevice->DeviceDescriptor.idProduct);
			return;
		}
#endif
