...
MUSB_Controller* MUSB_NewController(MUSB_SystemUtils* pUtils,
				    uint16_t wControllerType, int dwHighSpeed, int dwSendSrp,
				    void* pControllerAddressIsr,
				    ...
				    )
{
	...
	case MUSB_CONTROLLER_MHDRC:
	    MUSB_DIAG1(2, "\n\nInitializing MHDRC at ", (uint32_t)pControllerAddressIsr, 16, 8);
	    /* endpoint config discovery, etc. */
		...
		if (dwSendSrp == 0)
			pWrapper->PortImpl.bSendSrp = FALSE;
		else
			pWrapper->PortImpl.bSendSrp = TRUE;
	...
	{
	    MUSB_DIAG_STRING(2, "  High-bandwidth isochronous Rx support");
	}
	if(pWrapper->PortImpl.bHasHbIsoTx)
	{
	    MUSB_DIAG_STRING(2, "  High-bandwidth isochronous Tx support");
	}
