...
#define USB_STOR_TRANSPORT_EIO    -3
...
static int usb_get_max_lun(struct us_data *us)
{
	int len;
...
static int usb_stor_probe_device(struct usb_device *dev)
{
	...
	if (usb_storage_probe(dev, 0, &usb_stor[usb_max_devs])) {
		...
		for (lun = 0;
			...
			res = usb_stor_get_info(dev, &usb_stor[start],
...
static int usb_stor_BBB_transport(ccb *srb, struct us_data *us)
{
	...
	if (result == -EIO) {
		debug("failed to send CBW -EIO\n");
		return USB_STOR_TRANSPORT_EIO;
	}
	if (result < 0) {
		debug("failed to send CBW status %ld\n",
		      us->pusb_dev->status);
		result = usb_stor_BBB_reset(us);
		if (result == USB_STOR_TRANSPORT_EIO)
			return result;
		else
			return USB_STOR_TRANSPORT_FAILED;
	}
...
static int usb_inquiry(ccb *srb, struct us_data *ss)
{
	...
	do {
		...
		if (i == 0 || i == USB_STOR_TRANSPORT_EIO)
			break;
	} while (--retry);
...
static int usb_test_unit_ready(ccb *srb, struct us_data *ss)
{
	...
	do {
		...
		srb->cmdlen = 12;
		res = ss->transport(srb, ss);
		if (res == USB_STOR_TRANSPORT_GOOD) {
			ss->flags |= USB_READY;
			return 0;
		}
		if (res == USB_STOR_TRANSPORT_EIO) {
			return res;
		}
		res = usb_request_sense(srb, ss);
		if (res == USB_STOR_TRANSPORT_EIO) {
			return res;
		}
		/*
...
retry_it:
		...
		srb->pdata = (unsigned char *)buf_addr;
		res = usb_read_10(srb, ss, start, smallblks);
		if (res == USB_STOR_TRANSPORT_EIO) {
			info("Read ERROR EIO\n");
			blkcnt -= blks;
			break;
...
retry_it:
		...
		srb->datalen = usb_dev_desc[device].blksz * smallblks;
		srb->pdata = (unsigned char *)buf_addr;
		res = usb_write_10(srb, ss, start, smallblks);
		if (res == USB_STOR_TRANSPORT_EIO) {
			info("Write ERROR EIO\n");
			blkcnt -= blks;
			break;
		}
		if (res < 0) {
		      ...
		      block_dev_desc_t *dev_desc)
{
	...
	debug("ISO Vers %X, Response Data %X\n", usb_stor_buf[2],
	      usb_stor_buf[3]);
	res = usb_test_unit_ready(pccb, ss);
	if (res == USB_STOR_TRANSPORT_EIO) {
		info("usb_stor_get_info USB_STOR_TRANSPORT_EIO\n");
		return -1;
	}
	if (res) {
	...
	res = usb_read_capacity(pccb, ss);
	if (res == USB_STOR_TRANSPORT_EIO) {
		info("%s: usb_read_capacity() failed USB_STOR_TRANSPORT_EIO\n", __func__);
		return -1;
	}
	if (res != 0) {
