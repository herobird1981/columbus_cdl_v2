...
static int usb_hub_configure(struct usb_device *dev)
{
	...
	if (ret < 0) {
		info("usb_hub_configure: failed to get hub " \
	...
	if (sizeof(struct usb_hub_status) > USB_BUFSIZ) {
		info("usb_hub_configure: failed to get Status - " \
		      "too long: %d\n", descriptor->bLength);
		return -EFBIG;
	}
	...
	if (ret < 0) {
		info("usb_hub_configure: failed to get Status %x\n",
		      dev->status);
		return ret;
	}
	...
	for (i = 0; i < dev->maxchild; i++) {
		...
		info("Port %d Status %x Change %x\n",
		      i + 1, portstatus, portchange);

		if (portchange & USB_PORT_STAT_C_CONNECTION) {
			info("port %d connection change\n", i + 1);
