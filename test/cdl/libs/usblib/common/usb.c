			...
			void *data, unsigned short size, int timeout)
{
	...
	if (!((USB_REQ_GET_STATUS == request) && (requesttype == 0xa3)))
		info("host setup: req%02x.%02x v%04x i%04x l%d\n",
...
static int get_descriptor_len(struct usb_device *dev, int len, int expect_len)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (err < expect_len) {
			if (err < 0) {
				printf("unable to get device descriptor retry:%d (error=%d)\n", i, err);
				mdelay(500);
				ret = err;
			} else {
				printf("USB device descriptor short read retry:%d (expected %i, got %i)\n",
						i, expect_len, err);
				ret = -EIO;
			}
		} else {
			      ...
			      struct usb_device *parent)
{
	...
	for (i = 0; i < GET_DESCRIPTOR_TRIES; (++i, mdelay(200))) {
		r = usb_setup_descriptor(dev, do_read);
		err = usb_hub_port_reset(dev, parent);
		if (err)
			return err;
		if (r) {
			printf("device descriptor read/64, retry:%d error %d\n", i, r);
			continue;
		} else {
			break;
		}
	...
	if (r) {
		printf("device descriptor read/64, failed finally\n");
		return r;
	}
	dev->devnum = addr;
