...
{
	void *mbase;
	/* wait up to 5 seconds for a device connection is stable*/
	...
	if (get_timer(0) >= timeout) {
		dev_info(musb->controller, "musb_lowlevel_init: devctl %02x power: %02x\n",
				musb_readb(mbase, MUSB_DEVCTL), musb_readb(mbase, MUSB_POWER));
		return -ENODEV;
	}
