	...
	if (list_empty(&qh->hep->urb_list)) {
		...
		case USB_ENDPOINT_XFER_BULK:
			...
			if (qh->mux == 1) {
				...
				list_del(&qh->ring);
				kfree(qh);
				dev_dbg(musb->controller, "musb_advance_schedule non-periodic kfree qh: %p\n", qh);
				qh = NULL;
				qh = first_qh(head);
				break;
			}
		...
		case USB_ENDPOINT_XFER_INT:
			 ...
			 */
			kfree(qh);
			dev_dbg(musb->controller, "musb_advance_schedule periodic kfree qh: %p\n", qh);
			qh = NULL;
			break;
		}
	}
...
void musb_host_rx(struct musb *musb, u8 epnum)
{
	...
	if (rx_csr & MUSB_RXCSR_H_RXSTALL) {
		...
		dev_info(musb->controller, "end %d RX proto error -EPROTO\n", epnum);
	...
	gfp_t				mem_flags)
{
	...
	if (!qh) {
		dev_info(musb->controller, "musb_urb_enqueue not enough memory\n");
		spin_lock_irqsave(&musb->lock, flags);
		usb_hcd_unlink_urb_from_ep(hcd, urb);
		spin_unlock_irqrestore(&musb->lock, flags);
		return -ENOMEM;
	}
	dev_dbg(musb->controller, "musb_urb_enqueue kzalloc qh: %p\n", qh);
	...
	if (hep->hcpriv) {
		 ...
		 */
		kfree(qh);
		dev_dbg(musb->controller, "musb_urb_enqueue kfree qh: %p\n", qh);
		qh = NULL;
		ret = 0;
	} else
	...
	if (ret != 0) {
		...
		spin_unlock_irqrestore(&musb->lock, flags);
		kfree(qh);
		dev_dbg(musb->controller, "musb_urb_enqueue done kfree qh: %p\n", qh);
		qh = NULL;
	}
	return ret;
}
	...
	int status)
{
	...
	if (!qh->is_ready
		...
		if (ready && list_empty(&qh->hep->urb_list)) {
			...
			list_del(&qh->ring);
			kfree(qh);
			dev_dbg(musb->controller, "musb_urb_dequeue kfree qh: %p\n", qh);
			qh = NULL;
		}
	} else
